...
import { Pdfdata, DebtCollData } from "../PdfData";
        ...
        <div className="flex-1">
          <div className="flex flex-col  sm:gap-5 gap-3">
            <h2 className="lg:text-[32px] sm:text-2xl text-xl roboto-serif-font font-semibold text-primary text-left pt-4">
              Debt Collection and Revenue Management Testimonials
            </h2>
            <h3 className="text-primary sm:text-xl text-sm font-semibold roboto-serif-font">
              With Proven Results
            </h3>
          </div>
          <ul className=" mt-10 flex flex-col lg:gap-[50px] gap-8">
            {DebtCollData.slice(0, 43).map((data, index) => (
              <li key={index} className="flex bg-white">
                <Link to="/debtcollinner" className="lg:text-xl sm:text-lg text-sm roboto-serif-font text-blacklight">
                  {data.title}
                </Link>
              </li>
            ))}
